...
function onPaletteChange(e: any) {
...
function onLowResolutionClick(e: any) {
...
function onFullscreenChange(isFullscreen: boolean) {
  if (isFullscreen) {
    const { width, height, canvasWidth, canvasHeight, top, left } =
      getFullscreenSize();
...
function bind_events() {
  ...
  game.ondblclick = (e: any) => {
    // for some odd reason, doing a simple touchstart/end on
    // a joystick right after going fullscreen will generate
    // a dblclick event, exiting fullscreen
    // This dblclick event is generated by the virtual joystick's
    // div element
    ...
    if (!IS_IOS) {
      ...
      onFullscreenChange(is_fullscreen);
    }
  };

  touch_manager = nipplejs.create({
    zone: document.getElementsByClassName("square")[0] as any,
    // mode: 'static',
    position: {
      left: "50%",
      top: "50%",
  ...
  let button = nipplejs.create({
    zone: document.getElementsByClassName("square2")[0] as any,
    threshold: 0,
    // mode: 'static',
    lockX: true,
    lockY: true,
  ...
  (button as any).on("start end", (evt: any) => {
    let id = 0;
    if (evt.type === "start") {
      if (id) {
        clearTimeout(id);
...
function is_key_pressed(code: number) {
  ...
  0x19: function () {
    ...
    if (num > 16000) {
      ...
      if (num >= 3000) {
        ...
        const bitmap = Res.bitmaps[num] as any;
...
function draw_string(num: number, color: number, x: number, y: number) {
  let strings: Record<string, string> = Res.strings_en;
...
function change_palette(num: number) {
  palette_type = num;
}
...
function change_language(num: number) {
  strings_language = num;
}

function set_1991_resolution(low: boolean) {
  is_1991 = low;
}

function update_screen(offset: number) {
...
function play_sound(
  ...
  try {
    if (Res.sounds[resNum]) {
      const [, , me] = Res.sounds[resNum] as any;
      if (me) {
        // assert(freq < 40);
        if (freq >= 40) {
...
function load_modules() {
  Object.entries(Res.modules).forEach(([, module]: any[]) => {
...
function load_sounds() {
  Object.entries(Res.sounds).forEach(([, sound]: any[]) => {
